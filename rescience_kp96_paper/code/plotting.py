...
def _plot_MI_subplot(metric, number, function, X, Y, results):
    ...
    ax.set_xlabel('r', labelpad=15, fontsize=14)
    ax.set_ylabel('c', labelpad=15, fontsize=14)
...
def plot_fig2(X, Y, results):
    ...
    _plot_MI_subplot(b'I_X_C__R', 1, b'additive', X, Y, results)

    # title, metric, number, X, Y, results, color
    _plot_MI_subplot(b'I_X_C__R', 2, b'modulatory', X, Y, results)

    # title, metric, number, X, Y, results, color
    _plot_MI_subplot(b'I_X_C__R', 3, b'both', X, Y, results)
...
def plot_fig3(X, analytical_results, simulation_results):
    ...
    ax.text(-0.04, -0.10, 'a', transform=ax.transAxes, fontsize=23, fontweight='bold')

    ax = plt.subplot(3, 1, 2)
    # ax.set_title("I(X;R|C)")
    #ax.set_xlabel('Sample size')
    ax.set_ylabel('Information bits', fontsize=21)
    ...
    ax.text(-0.04, -0.10, 'b', transform=ax.transAxes, fontsize=23, fontweight='bold')
    ...
    ax.set_xlabel('Sample size', fontsize=21)
    #ax.set_ylabel('Information bits')
    ...
    ax.text(-0.04, -0.10, 'c', transform=ax.transAxes, fontsize=23, fontweight='bold')
